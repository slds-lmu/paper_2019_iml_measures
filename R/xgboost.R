...
trainLearner.regr.xgboost.mod = function(.learner, .task, .subset, .weights = NULL,  ...) {
  ...
  net.rent = task.data$target
  task.data = cbind(net.rent, task.data$data)
  names(task.data[1]) = "net.rent"

  task.data = createDummyFeatures(task.data)
  target.data = task.data[, 1]
  task.data = task.data[, -1]


  parlist$data = xgboost::xgb.DMatrix(data = data.matrix(task.data),
    label = target.data)
...
predictLearner.regr.xgboost.mod = function(.learner, .model, .newdata, ...) {
  ...
  predict(m, newdata = data.matrix(createDummyFeatures(.newdata)), ...)
}
