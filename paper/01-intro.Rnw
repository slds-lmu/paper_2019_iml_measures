...
A restriction to those structurally simpler models has the drawback that better performing models are often excluded a priori from model selection.
An alternative is to allow any model and apply post-hoc interpretation methods to explain model behavior and predictions.
Post-hoc interpretation methods e.g. quantify effects that features have on predictions, compute feature importances or explain individual predictions, see \citep{molnar2019} for an overview.
