...
\textbf{Notation:} We consider machine learning prediction functions $\fh:\mathbb{R}^p \mapsto \mathbb{R}$, where $\fh(x)$ is a prediction (e.g., regression output or a classification score).
...
By integrating the expected derivative of $\fh$ with respect to $X_j$ the effect of $x_j$ on the prediction function $\fh$ is isolated from the effects of all other features.
