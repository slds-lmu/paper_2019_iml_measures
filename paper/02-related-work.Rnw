...
\subsubsection{Notation:} We consider machine learning prediction functions $\fh:x \mapsto y$, where $x \in \mathbb{R}^p$ is a p-dimensional feature vector and $y \in \mathbb{R}$ is the prediction (e.g. regression output or a classification score).
For the decomposition of this function, we write $f_S:x_S \mapsto y$, $S \subseteq\{1, \ldots, p\}$, $x_S \in \mathbb{R}^{|S|}$ to denote a function that maps a vector with a subset of features to a marginal prediction.
...
\subsubsection{Interpretability Measures:}
\label{sec:other}
% =============================================================================
...
% =============================================================================
In \citep{philipp2018measuring} the authors propose model-agnostic measures of model stability based on semantic similarity of predictions.
...
Varying constraints have been proposed that yield a unique decomposition \citep{stone1994use,hooker2007generalized,apley2016visualizing}.
...
For the estimation we refer to \citep{apley2016visualizing}.
% =============================================================================
% ALE is best
% =============================================================================
We base our proposed measures on the ALE decomposition, because ALE are computationally cheap (worst case $O(n)$ for a feature main effect), can be computed sequentially instead of simultaneously, do not require knowledge of the joint distribution and  have a software implementations \citep{iml,alepackage}.
