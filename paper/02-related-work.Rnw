...
\textbf{Notation:} We consider machine learning prediction functions $\fh:x \mapsto y$, where $x \in \mathbb{R}^p$ is a p-dimensional feature vector and $y \in \mathbb{R}$ is the prediction (e.g. regression output or a classification score).
For the decomposition of $\fh$, we write $f_S:x_S \mapsto y$, $S \subseteq\{1, \ldots, p\}$, $x_S \in \mathbb{R}^{|S|}$ to denote a function that maps a vector with a subset of features to a marginal prediction.
If subset $S$ contains a single feature, we write $f_j$.
...
\textbf{Interpretability Measures:}
\label{sec:other}
% =============================================================================
...
Model size is often used as a measure for interpretability (e.g. number of decision rules, tree depth, number of non-zero coefficients) \citep{huysmans2012empirical,ruping2006learning,askira1998knowledge,yang2017scalable,schielzeth2010simple}. 
...
In \citep{plumb2019regularizing} the authors propose explanation fidelity and stability of local explanation models \citep{ribeiro2016should}.
% =============================================================================
% Other approaches
% =============================================================================
Further approaches measure interpretability as the usability of a model to support a human in a task, usually measured in a survey as response time, correctness of the response and task difficulty \citep{zhou2018measuring,huysmans2011empirical,dhurandhar2017tip}.
In \citep{friedler2019assessing} an interpretability measure based on runtime operation count is proposed.
To our best knowledge there are no model-agnostic measures that describe the interaction strength, complexity of main effects and number of features.
...
\falej(x_j) = \int_{z_{0,j}}^{x_j} \mathbb{E}\left[\frac{\partial \fh(X_1,\ldots,X_p)}{\partial X_j}\middle|X_j = z_j\right]dz_j-c_j
\end{eqnarray}
...
ALE main effects are estimated with finite differences, i.e. access to the gradients of the model is not required.
For the estimation see \citep{apley2016visualizing}.
% =============================================================================
% ALE is best
% =============================================================================
We base our proposed measures on the ALE decomposition, because ALE are computationally cheap (worst case $O(n)$ per main effect), can be computed sequentially instead of simultaneously, does not require knowledge of the joint distribution and has software implementations \citep{iml,alepackage}.
