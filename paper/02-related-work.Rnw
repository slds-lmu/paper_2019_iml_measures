...
Model size is often used as a measure for interpretability (e.g. number of decision rules, tree depth, ...) \citep{huysmans2011empirical,ruping2006learning,askira1998knowledge,yang2017scalable}. 
...
In \citep{plumb2019regularizing} the authors propose explanation fidelity and explanation stability metrics of local explanation models \citep{ribeiro2016should}.
...
\end{eqnarray}
Here, $z_{0,j}$ is a lower bound of $X_j$ (usually the minimum observed value of $x_j$) and the expectation $\mathbb{E}$ is computed conditional on the value for $x_j$ and over the marginal distribution of all other features.
...
% ALE is best
% =============================================================================
We base our proposed measures on the ALE decomposition, because ALE are computationally cheap (worst case $O(n)$ for a feature main effect), the effects can be computed sequentially instead of simultaneously as in \citep{hooker2007generalized} and, most importantly, ALEs do not require knowledge of the joint data distribution.
Additionally, ALE have software implementations \citep{iml,alepackage}.
