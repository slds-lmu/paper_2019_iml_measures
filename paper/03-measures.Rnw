...
% ============================================================================
Based on Equation~\ref{eqn:decomp}, we decompose the prediction function into a constant (estimated as $f_0 = \frac{1}{n}\sum_{i=1}^n \fh(\xi)$), plus the main effects (estimate with ALE), and a remainder term containing interactions (difference between full model and $f_0$ + main effects).
