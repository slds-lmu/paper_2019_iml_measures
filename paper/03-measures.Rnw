...
% ============================================================================
Based on Equation~\ref{eqn:decomp}, we decompose the prediction function into a constant (estimated as $f_0 = \frac{1}{n}\sum_{i=1}^n \fh(\xi)$), plus the main effects (estimate with ALE), and a remainder term containing interactions (difference between full model and constant plus sum of main effects).
...
\end{eqnarray}
This arrangement of components emphasizes a decomposition of the prediction function into a main effect model and interaction remainder.
The main effect model itself can be used as a prediction function and we can analyze how well it approximates $f$, which is the idea behind the interaction measure IAS.
