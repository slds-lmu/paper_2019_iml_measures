...
We refer to the training data for the machine learning model with the tuples $\mathbb{D} = \{x^{(i)},y^{(i)}\}_{i=1}^n$ and refer to the value of the j-th feature from the i-th instance as $x_j^{(i)}$.
