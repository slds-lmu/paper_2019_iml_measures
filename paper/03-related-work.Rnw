...
As related work we see other approaches for measuring model complexity for interpretability and methods for decomposing functions on which we build (which is the background).
...
In scalable bayesian rule lists \citep{yang2017scalable}, sparsity will be measured as the number of leaves in a decision tree or as the number of rules in a rule list.
...
AIC \citep{akaike1998information}, BIC \citep{schwarz1978estimating} are tradeoff measure between accuracy and interpretability.
Both measures require to know the number of model parameters.
...
% Other approaches
% =============================================================================
TIP: Typifying the Interpretability of Procedures => They suggest to measure improvement in helping with some kind of task. \citep{dhurandhar2017tip}
% Measruing interpretability through users.
\citep{friedler2019assessing} measure with users if models are intepretable and proopose as objective measure the runtime operation count of the model.
We take a different approach because we look at prediction function from the outside, not into the model structure and parameters.
...
Towards A rigourous science ... \citep{doshi2017towards}
