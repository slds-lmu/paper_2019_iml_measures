...
We review approaches for quantifying interpretability and summarize the background on functional decomposition as the basis for our proposed interpretability measures.
...
\citep{zhou2018measuring} propose a general form for model interpetability which includes the structural complexity of the model but again compare model-specific measures like number of leaves, average depth and maximum depth of a tree. 

Akaikes Information Criterion (AIC)  \citep{akaike1998information} and the  Bayesian Information Criterion (BIC)  \citep{schwarz1978estimating} are more broadly applicable tradeoff measure between goodness of fit and interpretability.
...
Also the tradeoff between interpretability and performance is fixed and only one dimension of interpretability is considered, i.e. degrees of freedom.
...
Dhurandhar et. al 2018 \citep{dhurandhar2017tip} propose to measure interpretability as improvement in the task the machine learning model originally tries to solve
...
For example the representation of a tree can become more complex, while the functional complexity decreases (because the tree approximates a linear function).
...
Without constraints, there is no unique decomposition and the components have no guarantee to be meaningful. 
...
% ALE decompositions
% =============================================================================
Another approach is Accumulated Local Effects (ALE)proposed by \cite{apley2016visualizing} as a tool for visualizing feature effects (see e.g. Figure~\ref{fig:c-demo}), that also represents a decomposition.
