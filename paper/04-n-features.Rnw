...
If available, model-specific extraction of number of feature used is preferable.
The model-agnostic version is useful when e.g. the prediction function is only accessible via API or when the feature transformation pipeline is very complex.
