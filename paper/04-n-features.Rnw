...
NF = 0\;
	\For{$j \in 1,\ldots,p$}{
		Draw $M$ instances $\{x^{(m)}\}_{m=1}^M$ from dataset $\D$\;
			Create $\{x^{(m)*}\}_{m=1}^M$ as a copy of $\{x^{(m)}\}_{m=1}^M$ and replace values for the j-th feature by randomly sampling $x_j$ from $\D$\;
...
lrn3c = makeLearner("regr.rpart", maxdepth = 3)
lrn4c = makeLearner("regr.rpart", maxdepth = 4)
lrn5c = makeLearner("regr.rpart", maxdepth = 5)
lrn6c = makeLearner("regr.rpart", maxdepth = 6)
...
lrns = list(lrn1c, lrn2c, lrn3c, lrn4c, lrn5c, lrn6c, lrn1, lrn2, lrn3, lrn4, lrn5, lrn6)
...
aggr = dfs[,.(med = round(mean(abs(NF - NF.true)),1)),by = c("m", "model")]

#ggplot(data.frame(dfs)) +
#  geom_boxplot(aes(x = factor(m), y = NF.true - NF, group = factor(m)))
@

We tested the NF heuristic on the Boston Housing data, on which we trained decision trees (CART) with maximum depths from 1 to 6 resulting in \Sexpr{nf_tree(mods[[1]])}, \Sexpr{nf_tree(mods[[2]])}, \Sexpr{nf_tree(mods[[3]])}, \Sexpr{nf_tree(mods[[4]])}, \Sexpr{nf_tree(mods[[5]])} and \Sexpr{nf_tree(mods[[6]])} number of features used and a LASSO model with penalization $\lambda\in\{10, 5, 2, 1, 0.1, 0.001\}$ resulting in \Sexpr{nf_glmnet(mods[[7]])}, \Sexpr{nf_glmnet(mods[[8]])}, \Sexpr{nf_glmnet(mods[[9]])}, \Sexpr{nf_glmnet(mods[[10]])}, \Sexpr{nf_glmnet(mods[[11]])} and \Sexpr{nf_glmnet(mods[[12]])} number of features. 
We estimated NF, the number of features with sample sizes 10, 50, 500 and repeated each \Sexpr{n_repetitions} times.
For elastic net, NF was always correct, independent of $m$.\Sexpr{stopifnot(all(aggr[model == "elastic net", med] == 0))}
For CART and $m\in\{10, 50, 500\}$ the mean absolute error between NF and the true number of features used were \Sexpr{paste(aggr[model == "CART", med], collapse = ", ")}.
