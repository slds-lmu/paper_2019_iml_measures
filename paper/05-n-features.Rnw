...
The heuristic to compute whether a feature $x_j$ was used is described in Algorithm~\ref{algo:nfeat}.
...
NF = 0\;
	\For{$j \in 1,\ldots,p$}{
		\For{$k=1,\ldots,K$}{
			...
			For each instance, sample feature value $\xj$ from $\xjvec$\;
...
$$\mathbb{P}(\hat{z}_j = 0| z_j = 1) = (0.99 + 0.01 * 0.01^10)^100 = 0.3660$$
