...
NF = 0\;
	\For{$j \in 1,\ldots,p$}{
		Draw $M$ instances $\{x^{(m)}\}_{m=1}^M$ from dataset $D$\;
			Create $\{x^{(m)*}\}_{m=1}^M$ by replacing $x_j$ with samples from $X_j$ in $\{x^{(m)}\}_{m=1}^M$ \;
...
When we set $M=100$, then $\mathbb{P}(\hat{z}_j = 0| z_j = 1) = (0.99 + 0.01 * 0.01)^{100} \approx 0.37$.
