...
Estimate ALE $\falej$\;
	...
	\While{$K < max_{seg}$ AND $R^2 < (1- \epsilon)$}{
		...
		\tcp{For categorical feature, fix slopes to zero} 
		$g_j(x_j) = \sum_{k=1}^{K+1} \I_{\xi\in Z_k}\cdot\left(\hat{\beta}_{0,k} + \hat{\beta}_{1,k}\xij\right)$
	...
	\For{$k \in 1,\ldots,K$}{
		Set $\beta_{0,k} = \frac{1}{\sum_{i=1}^n\I_{\xi\in Z_k}}\sum_{i=1}^n\I_{\xi\in Z_k}\cdot\falej(\xij)$ and $\beta_{1,k} = 0$ in $g_j$\;
