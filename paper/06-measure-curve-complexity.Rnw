...
fc = FunComplexity$new(pred, epsilon = 0.001, grid.size = 200, max_seg_num = 5)
...
coef_diffs = abs(earth_coefs - coefs_approx_df[,colnames(earth_coefs)])
mean(coef_diffs > 0.01)

## TODO: Make post-pruning optional, and switch off here


cosines = angle(unlist(earth_coefs), unlist(coefs_approx_df[,colnames(earth_coefs)]))
cosines = angle(unlist(coefs_approx_df[,colnames(earth_coefs)]), unlist(earth_coefs))
quantile(cosines, probs = seq(from = 0, to = 1, length.out = 10))
