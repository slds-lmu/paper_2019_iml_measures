...
For a single ALE main effect, we define $\text{MEC}_j$ as the number of parameters needed to approximate the curve with linear segments.
...
In the remainder, main effect ALE curves are represented by sparklines, e.g., Figure~\ref{fig:c-demo} by \Sexpr{spark(app$ale)}.
Vertical bars, if present (e.g., \Sexpr{spark(app)}) show borders between the segments of an approximation.
%
...
% Measure Intuition
% =============================================================================
Through the approximation with linear segments, the degrees of freedom required for describing a main effect curve become measurable.
We measure the degrees of freedom as the number of non-zero coefficients for intercepts and slopes of the linear segments.
The approximation allows some error, e.g., an almost linear main effect like \Sexpr{spark(fc1$approx_models$V3$ale)} may have $\text{MEC}_j=1$, even if dozens of parameters would be needed to describe it perfectly.
The approximation quality is measured with R-squared, i.e., the proportion of variance of $\falej$ that is explained by the approximation with linear segments.
...
We apply a post-processing step that greedily sets slopes of the linear segments to zero, as long as R-squared $\in\{1-\epsilon,1\}$.
This post-processing step potentially decreases the $\text{MEC}_j$, especially for models with constant segments like decision trees or rule-based models.
% =============================================================================
...
\caption{Main Effect Complexity (MEC).}\label{algo:AMEC}
	\KwInput{Model $\fh$, approximation error $\epsilon$, max. segments $max_{seg}$, data $\D$}
	...
	\While{$K < max_{seg}$ AND $R^2(g_j,\falej) < (1- \epsilon)$}{
		\tcp{Optimize intervals $Z_k$ via generalized simulated annealing, estimate $\beta$'s per segment with ordinary least squares}
