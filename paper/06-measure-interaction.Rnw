...
\subsection{Measure ALE First Order Approximiation}
...
$$PRL(f, f_{ALE}, L, f_0) = 1 - \frac{\mathbb{E}(L(f, f_ALE))}{\mathbb{E}(L(f, c))}$$
...
$$SSE = \sum_{i=1}^n (f(x_i) - f_{1st,ALE}(x_i))^2$$

$$SST = \sum_{i=1}^n (f(x_i) -  \frac{1}{n}\sum_{i=1}^n f(x_i))^2$$
