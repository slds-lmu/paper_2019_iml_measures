...
\subsubsection{Fewer features, less verbosity.}
...
As demonstrated in Table~\ref{tab:spark-table-multiobj}, a model with fewer features has a more compact representation and if additionally $IAS=0$, the ALE main effects fully characterize the prediction function.
Interpretation methods that analyze 2-way feature interactions scale with $O(\text{NF}^2)$.
A complete functional decomposition \citep{hooker2007generalized,apley2016visualizing} would require to estimate $\sum_{k=1}^{NF} {NF\choose k}$ components which has a computational complexity of $O(2^{NF})$.

\subsubsection{Less interaction, more reliable feature effects.}
...
We simulated \Sexpr{n} data points with 4 features and a continuous target based on Friedman et. al (1991)\citep{friedman1991multivariate}.
...
Summarizing the effects with a single number (e.g. average marginal effects \citep{leeper2017interpreting}) can be misleading, e.g. if the effect has a U-shape, the average effect might be zero.
