...
% Measure Intro
% =============================================================================
We propose a measure that, for each of the main effects, approximates the ALE curve with linear segments and adds up the parameters needed if we were to model all those segments in a segmented linear regression model, which models the (approximate) ALE curve as a function of the single feature.
...
app = AleNumApprox$new(ale = fe, max_seg = 6, epsilon = 0.05)
p1 = plot(app)  + scale_y_continuous(limits = c(-7,7))
...
fe = FeatureEffect$new(pred, "x", grid.size = grid.size)
app = AleNumApprox$new(ale = fe, max_seg = 5, epsilon = 0.05)
p2 = plot(app) + scale_y_continuous(limits = c(-7,7))
...
fe = FeatureEffect$new(pred, "x", grid.size = grid.size)
app = AleNumApprox$new(ale = fe, max_seg = 6, epsilon = 0.05)
p3 = plot(app)  + scale_y_continuous(limits = c(-7,7))
...
\begin{enumerate}
\item Input: Model $\fh$, PRL-bound $\epsilon$, complexity limits $max_{segcat}$ and $max_{segnum}$
...
\begin{itemize}
\item $\galej = FindBestSplit(K, \falej, \epsilon)$
\end{itemize}
...
% =============================================================================
\begin{algorithm}
\caption{Find best splits for given k.}\label{algo:ale-complexity-splits}
\begin{enumerate}
\item Input: K, $\falej$, $\epsilon$, Number of samples $m$
\item If numeric: $\hat{z}_1,\ldots, \hat{z}_K = argmin_{z_1,\ldots,z_K} \sum_{k=1}^{K+1} \sum_{\xi\in Z_k}\left(\falej(\xij) - (\beta_{j0}^k + \beta_{j1}^k\xij)\right)^2$ where $Z_1 = [min(\xj), z_1], \ldots, Z_k = [z_{k-1}, z_k],\ldots,Z_{K+1} = [z_K,max(\xj)]$ and $\beta_{j0}^k$ and $\beta_{j1}^k$ are the least square estimates. If $K=0$, then $Z_1 = [min(\xj), max(\xj)]$. Solve optimization with Generalized Simulated annealing (package GenSA).
\item If categorical: Do $m$ times random search of split points and estimate mean in each segment.
...
fc$plot(fc$relevant_features, nrow = 1)
@
...
<<depends = "fun-complex", fig.cap = "Effect of different epsilons on complexity of approximation. On the left the ALE plot can be approximated with a single linear segment for the given epsilon of 0.05. On the right two segments are needed to approximate the ALE curve when epsilon is set to 0.001.">>=
fc = FunComplexity$new(pred.ksvm, epsilon = 0.05,  grid.size = grid.size)
p1 = plot(fc$approx_models$V3)

fc = FunComplexity$new(pred.ksvm, epsilon = 0.001, grid.size = grid.size, max_seg_num = 10)
