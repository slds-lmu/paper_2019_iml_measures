...
For categorical features we do an exhaustive search with through all possible n.breaks:
...
fc = FunComplexity$new(pred.ksvm, epsilon = 0.001, grid.size = grid.size)
...
<<depends = "fun-complex">>=
fc.ksvm = FunComplexity$new(pred.ksvm, epsilon = 0.05,  grid.size = grid.size)
...
fc.rpart = FunComplexity$new(pred.rpart, epsilon = 0.05,  grid.size = grid.size)
