...
dat1$V1 = cut(dat1$V1, breaks = c(0, 0.1, 0.5, 0.7, 1), include.lowest = TRUE)
dat1$test = factor(c("a", "b"))
...
fc = FunComplexity$new(pred, epsilon = 0.05, max_feat_cost = 10)
fc$plot_complexity("V10")
fc$var_explained
fc$complexity_total
...
aa = AleNumApprox$new(ale, 0.1, 10)
aa$predict(dat1)
aa$r2
aa$n_coefs
aa$model
aa$plot()
aa = AleCatApprox$new(ale, 0.1, 10)
aa$transform(dat1)
dim(aa$transform(dat1))
aa$predict(dat1)
aa$r2
aa$n_coefs
aa$max_complex
plot(aa)
mods = fc$.__enclos_env__$private$approx_models
