...
<<epsilons, depends = "fun-complex", fig.cap = "Effect of different epsilons on complexity of approximation. On the left the ALE plot can be approximated with a single linear segment for the given epsilon of 0.05. On the right two segments are needed to approximate the ALE curve when epsilon is set to 0.001.">>=
eps1 = 0.05
...
<<spark-table, dependson="fun-complex", results="asis", eval=FALSE>>=
