...
\begin{enumerate}
\item Input: Model $\fh$, PRL-bound $\epsilon$, complexity limit $c_{max}$
\item Output: Weighted average complexity $C$
\item For all features $\xj, j \in \pset$, do:
\begin{enumerate}
\item Estimate ALE $\falej$
\item If $\falej(x_j) = 0$ for all $x$, stop and return $\galej(x) = 0$
\item If $\xj$ numeric: Estimate $\galej(\xj) = \hat{\beta_0} + \hat{\beta}\xj$ with $\argmin_{\beta} \sum_{i=1}^n \left(\falej(\xij) - (\beta_0 + \beta_j\xij)\right)^2$.
If $PRL(\falej, \galej, L) < \epsilon$, return $C = 1$.
\item For $k \in \{1,\ldots,max_{seg}\}$:
\begin{itemize}
\item If feature $x_j$ is categorical, order categories $cat_{l}^j, l \in \{1,\ldots,L_j\} $by increasing $f_{j, ALE}(x)$.
...
\end{itemize}
\item $c_j$ = Number of weights needed in linear regression model ($m_j$)
...
\item $c_j = min(c_{max}, c_j)$
\item $V_j = \frac{1}{n}\sum_{i=1}^n \hat{f}^{ALE}_j(x^{(i)})$
\end{enumerate}
\item $c$ is weighted mean: $c = \frac{1}{\sum_{j=1}^pV_j}\sum_{j=1}^p V_jc_j$.
\end{enumerate}
\end{algorithm}
