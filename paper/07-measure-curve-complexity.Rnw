...
For an individual ALE main effect, $\text{MEC}_j$ is the number of parameters needed to approximate the curve with linear segments.
For the entire model, MEC is the average $\text{MEC}_j$ over all main effects, weighted by their variance.
Figure~\ref{fig:c-demo} shows an ALE plot (= main effect) and its approximation with two linear segments.
In the remainder, main effect ALE curves will be represented by sparklines, e.g. Figure~\ref{fig:c-demo} by \Sexpr{spark(app)}.
Vertical bars -- if present -- show intervals between segments of the approximation.
...
% Measure Intuition
% =============================================================================
Through the approximation with linear segments, the degrees of freedoms needed for describing a main effect curve becomes measurable.
It also allows some approximation error, e.g. an almost linear main effect like \Sexpr{spark(fc1$approx_models$V3$ale)} 
 has $\text{MEC}_j=1$, even if dozens of parameters would be needed to perfectly describe it. 
The approximation quality is measured with the R-squared measure.
...
In the case that an approximation can't reach an R-squared above $1-\epsilon$ with a given $max_{seg}$, $\text{MEC}_j$ is computed with the maximum number of segments.
Approximation error $\epsilon$ should be small, but not too small.
We found approximations with $\epsilon$ between $0.01$ and $0.1$ visually meaningful.
% =============================================================================
% Measure Sum Up and Fix
% =============================================================================
$\text{MEC}_j$ is averaged over all features to get the main effect complexity for the model.
The individual $\text{MEC}_j$ are weighted by the variance of the corresponding ALE main effect, to give less weight to features that did not contribute much to the prediction, e.g. when their ALE curve wiggles around zero.
...
For example the three main effect curves \Sexpr{spark(app1$ale, c(-6, 6), width = 5)}, \Sexpr{spark(app2$ale, c(-6, 6), width = 5)} and \Sexpr{spark(app3$ale, c(-6, 6), width = 5)} differ by a scaling factor in the y-axis, yet each needs five segments to be approximated with $R^2 >= 0.95$:
...
\caption{Main Effect Complexity (MEC).}\label{algo:AMEC}
	...
	\KwOutput{Main Effect Complexity MEC}
	\For{$j \in \pset$}{
Estimate ALE $\falej$\;
