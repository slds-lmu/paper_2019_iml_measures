...
\begin{enumerate}
\item $max_{seg,j} = c_{max}/2$ if numerical, $max_{seg,j} = c_{max}$ if categorical.
...
\begin{itemize}
\item $\hat{z}_1,\ldots, \hat{z}_K = argmin_{z_1,\ldots,z_K} \sum_{k=1}^{K+1} \sum_{\xi\in Z_k}\left(\falej(\xij) - (\beta_0^k + \beta_j^k\xij)\right)^2$ where $Z_1 = [min(\xj), z_1], \ldots, Z_k = [z_{k-1}, z_k],\ldots,Z_{K+1} = [z_K,max(\xj)]$. If $K=0$, then $Z_1 = [min(\xj), max(\xj)]$. In case of categorical, $\beta_j$ is set to zero. Solve optimization with Generalized Simulated annealing (package GenSA).
\item $\galej(\xj) = \sum_{k=1}^{K+1} \I_{\xi\in Z_k}\left(\hat{\beta}_0^k + \hat{\beta}_j^k\xij\right)$. For categorical, $\hat{\beta}_j = 0$
\end{itemize}
\item $C_j = 2 * K$ for numerical; $C_j = K$ for categorical
