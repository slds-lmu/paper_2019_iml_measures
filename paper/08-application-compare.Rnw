...
plot(fc_rf, nrow = 2, rug = FALSE)

fc_rf$var_explained
fc_rf$complexity_total
...
fc_lm$var_explained
fc_lm$complexity_total
plot(fc_lm, nrow = 2, rug = FALSE)
@
...
dd = data.frame(y = newdata$quality, prediction = predictions, predictions_approx = predictions_approx)
