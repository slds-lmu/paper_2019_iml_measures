...
It is difficult to know the desired compromise between model complexity and performance before modeling the data and suggests multi-objective optimization \citep{freitas2014comprehensible}.
A solution is multi-objective optimization.
We suggest searching over a wide spectrum of model classes and hyper parameter settings and allow the practitioner to choose a suitable compromise between model complexity and performance.
The three complexity measures provide a detailed characterization of machine learning models, which enables making informed decisions (e.g. how much does performance suffer if we use a model without interactions?).
% =============================================================================
% Objectives and Models
% =============================================================================
We used the mlrMBO model-based optimization framework \citep{horn2016multi} with ParEGO \citep{knowles2006parego} (\Sexpr{max.iters} iterations) to find the best model based on four objectives: number of features used by the model (NF), main effect complexity (MEC), interaction strength (IAS) and the cross-validated mean absolute error (MAE) (\Sexpr{n.folds}-fold cross-validated).
...
colnames(tab) = best.models.print$model[indices]
...
cap = sprintf("A selection of four models from the Pareto optimal set, along with their ALE main effect curves. From left to right, the columns show models with 1) lowest MAE, 2) lowest MAE when $MEC=1$, 3) lowest MAE when $IAS =\\leq 0.1$, and 4) lowest MAE with $NF \\leq 7$.", length(indices))
