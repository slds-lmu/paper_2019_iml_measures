...
We used the mlrMBO model-based optimization framework \citep{horn2016multi} to find the best model based on four objectives: number of features used by the model (NF), main effect complexity (MEC), interaction strength (IAS) and the mean absolute error (MAE) on unseen data.
We optimized over the space of following model classes (and hyperparameters): CART (maximum tree-depth, pruning cp), support vector machine (cost C and sigma), elastic net regression (regularization alpha and penalization lambda), gradient boosted trees (maximum depth, number of iterations), gradient boosted generalized additive model (number of iterations) and a random forest (mtry, number of trees, mininmal node size)..
...
lrn_model_map = c("xgboost" = "gbt", "rpart" = "CART", "featureless" = "median", 
		  "ksvm" = "svm", "gamboost" = "gamb", "glmnet" = "elastic net", "ranger" = "rf")
...
To combine the three measures in a single dimension in Figure~\ref{fig:perf-interpret-tradeoff}, we defined interpretability ad-hoc as: $Interpretability = 3 - (NF_{scaled} + IAS_{scaled} + MEC_{scaled})$
