...
cap = sprintf("A selection of four models from the Pareto optimal set. From left to right, the columns show models with 1) lowest MAE, 2) lowest MAE when $MEC \\leq 2$, 3) lowest MAE when $IAS =\\leq 0.1$, and 4) lowest MAE with $NF \\leq 7$. Corresponding hyperparameters can be found in Table~\\ref{tab:pareto}.", length(indices))
...
ggplot(plot.dat,aes(x = 1 - sMAE, y = 3 - (sNF + sMEC + sIAS))) +
  geom_point() + 
  geom_text_repel(aes(label = descr.ref), size = 7) +
