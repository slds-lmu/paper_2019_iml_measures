...
Minimizing the number of features (NF), the interaction strength (IAS) and the main effect complexity (AMEC) improves readability, reliability and summarizability of post-hoc interpretation methods like partial dependence plots, ALE plots, feature importance, interaction effects, local surrogate models, etc..
...
For example, if 100 features were used by the model, 100 feature effect plots have to be inspected.
Interpretation methods that analyze 2-way feature interactions, scale with $O(\text{NF}^2)$.
The computation time of a full functional decomposition \citep{hooker2007generalized,apley2016visualizing} scales with $O(\text{NF}!)$.

\subsubsection{The less interactions, the more reliable feature effect plots.}
Feature effect plots, like partial dependence plots and ALE plots visualize the marginal relationship between a feature and the prediction.
The estimated effects are averages over instances.
The effects of a feature can vary greatly for the isntances, even take on a different direction when the model models feature interactions.
...
The regression target was simulated as: $y = (x_1^2 + (x_2 \cdot x_3 - (1/(x_2 \cdot x_4)))^2)^{0.5} + e$, where $ e \sim N(0,125)$.
...
In linear models, a feature effect can be expressed with a single number, the regression coefficient or weight.
When the effects are non-linear the method of choice is visualization  \citep{apley2016visualizing,friedman2001greedy}.
Summarizing the effects to a single number (e.g. average marginal effects \citep{leeper2017interpreting}) can be misleading, e.g. when the effect is first positive, the negative, the average effect might be zero.
As a by-product of MEC, there is a third option: Instead of reporting a single number, the linear model for each segment can be reported
Minimizing MEC means preferring models with main effects that can be described with less numbers, offering a more compact model description.
