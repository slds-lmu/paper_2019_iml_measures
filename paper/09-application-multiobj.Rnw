...
ps.regr = makeModelMultiplexerParamSet(lrn.regr,
  ...
  regr.rpart = makeParamSet(
    makeIntegerParam("maxdepth", lower = 1, upper = 7)
  )
)
@
...
get_fn = function(model_i){
	model_i = model_i
	fn = function(par) {
	obj = single_obj(measure_df, par)	
	as.numeric(sum(obj < obj[model_i]))
	}
}

get_best_lambdas = function(model_i){
	fn = get_fn(model_i)
	res = GenSA(c(0.5,0.5,0.5,0.5), fn = fn, lower = c(0,0,0,0), upper = c(1,1,1,1),
		    control = list("threshold.stop" = 0, maxit = 200))
	res$par / sum(res$par)
}

lambda_perf = c(1,0, 0, 0)
best_perf = which.min(single_obj(measure_df, lambda_perf))
# MAE-best model with bounded MEC
ind_2 = which(best.models.print$MEC <= 2)[1]
lambda_2 = get_best_lambdas(ind_2)
best_2 = which.min(single_obj(measure_df, lambda_2))
# MAE-best model that models few interactions
ind_3 = which(best.models.print$IA <= 0.1)[1]
lambda_3 = get_best_lambdas(ind_3)
best_3 = which.min(single_obj(measure_df, lambda_3))
# Best model that does not use all features
ind_4 = which(best.models.print$NF <  sum(task$task.desc$n.feat))[1] 
lambda_4 = get_best_lambdas(ind_4)
best_4 = which.min(single_obj(measure_df, lambda_4))

indices = c(best_perf, best_2, best_3, best_4)
mbo_indices = best.models.print[indices,"index"]

tab = get_spark_table(mbo_obj = mbo.iml, indices =  mbo_indices, ylim = c(-2, 2), height = 5, width=10)
...
cap = sprintf("A selection of %i models from the Pareto optimal set. From left to right, the models with best MAE (%s), best MAE when MEC <=2 (%s), best MAE when IA <= 0.1 (%s), best MAE with NF < 7 (%s).",
  length(indices),
  paste(sprintf("%.0f", lambda_perf), collapse = ","),
  paste(sprintf("%.2f", lambda_2), collapse = ","),
  paste(sprintf("%.2f", lambda_3), collapse = ","),
  paste(sprintf("%.2f", lambda_4), collapse = ","))
xtab = xtable(tab, caption = cap)
