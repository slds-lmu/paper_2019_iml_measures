...
As \citep{freitas2014comprehensible} points out, it is difficult to know before modeling what the tradeoffs between interpretability and performance are and suggests multi-objective optimization based on Pareto dominance.
...
tab = get_spark_table(mbo_obj = mbo.iml, indices =  mbo_indices, ylim = c(-2, 2), height = 3, width=10, log_params = log_params)
