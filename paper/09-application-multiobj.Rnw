...
The goal is to predict wine quality on a scale from 0 to 10, assessed by the median of three blind ratings.

% =============================================================================
% Why like this?
% =============================================================================
\subsubsection{Motivation.}
As \citep{freitas2014comprehensible} points out, it is difficult to know tradeoffs between interpretability and performance before the modeling step and suggests multi-objective optimization.
...
We suggest to search over a wide spectrum of model classes and hyper parameter settings, present the set of Pareto optimal models and let the practitioner choose a suitable balance between interpretability and performance.
The three measures of interpretability provide an in-depth characterization of the machine learning models, allowing practitioners to make more informed decisions (e.g. how much does performance suffer when we use a model without interactions?).

% =============================================================================
% Objectives and Models
% =============================================================================
\subsubsection{Optimization Setup.}
We applied the model-based optimization framework \citep{mlrMBO} to find the best model based on four objectives: number of features used by the model (NF), main effect complexity (MEC), interaction strength (IAS) and the mean absolute error (MAE) on test data.
We optimized over the space of following model classes (and hyperparameters): CART (maximum tree-depth, pruning cp), a support vector machine (cost C and sigma), ridge regression for sparse linear models (regularization alpha and penalization lambda), gradient boosted trees (maximum depth, number of iterations), gradient boosted generalized additive model (number of iterations) a random forest with 1000 trees (no tuning).
\subsubsection{Model-based Optimization Setup.}
We used the ParEGO algorithm \citep{knowles2006parego} for the multi-objective optimization.
Within the fitness function, the MAE was estimated using \Sexpr{n.folds}-fold cross-validation and the other measures (NF, MEC, IA) were estimated in-sample.
We set the number of iterations of ParEGO to \Sexpr{max.iters}.
For all other parameters of the multi-objective optimization, we relied on the sensitive defaults provided by \citep{mlrMBO}.
...
If two models from the same class but with different hyperparameter had the exact same MAE, NF, IAS and MEC, we dropped one randomly from the Pareto set.
We also dropped constant models (e.g. ridge regression with strong penalization). 
...
The four selected models show different tradeoffs between the four measures.
% =============================================================================
...
tab = get_spark_table(mbo_obj = mbo.iml, indices =  mbo_indices, ylim = c(-1.1, 1.1), height = 3, width=10, log_params = log_params)
...
The complexity measures allows to study the trade-off between interpretability and performance across different model classes and hyperparameter settings.
First, we mapped each measure to the interval $[0,1]$ by scaling each measure with meaningful upper and lower bounds:

$$M_{scaled}(M) = \frac{M - M_{inf}}{M_{sup} - M_{inf}}$$

For MAE, we set $M_{inf}$ to the MAE when we predict the median and $M_{sup} = \sum_{i=1}^n|y_i - median(y)|$.
...
To combine the tree measures into a single dimension in  Figure~\ref{fig:perf-interpret-tradeoff} we defined interpretability ad-hoc as: $Interpretability = 3 - (NF_{scaled} + IAS_{scaled} + MEC_{scaled})$
...
Figure~\ref{fig:perf-interpret-tradeoff} maps each model and hyperparameter configuration from the Pareto set into the performance/intepretability space.
...
plot.dat = best.models.print[c("sMAE", "sNF", "sMEC", "sIAS", "descr")]
plot.dat = rbind(plot.dat, data.frame(sMAE = 1, sNF = 0, sMEC = 0, sIAS = 0, descr = "median"))
ggplot(plot.dat,aes(x = 1 - sMAE, y = 3 - (sNF + sMEC + sIAS))) +
  geom_point() + 
  geom_text_repel(aes(label = descr), size = 5) +
