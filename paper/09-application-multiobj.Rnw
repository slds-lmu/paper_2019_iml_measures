...
ps.regr = makeModelMultiplexerParamSet(lrn.regr,
  ...
  regr.cubist = makeParamSet(
    makeIntegerParam("rules", lower = 1, upper = 500)
  )
)
@
...
% TODO: Read more about ParEGO

<<mo-run, dependson=c("mo-data", "mo-learners", "mo-setup")>>=
...
xtable::xtable(best.models.print[c("descr", measure_names)], caption = "tab.cap", label = "tab:pareto")
...
% =============================================================================
Figure \ref{fig:mo-best-performance} shows the main effects of the best performing model.

<<mo-best-performance, dependson="mo-run">>=
## Extract parameters and refit best solutions
best.model = best.models.print[best.models$y_1 == min(best.models$y_1), ][1,]
best_index = best.model$index
...
The selected model is a \Sexpr{best.model$selected.learner} using \Sexpr{best.model$y_4} features, an MAE of \Sexpr{best.model$y_1}, \Sexpr{best.model$y_3} variance explained due to interactions and average complexity of \Sexpr{best.model$y_2}.

% =============================================================================
% Another interesting model
% =============================================================================
Figure \ref{fig:mo-interesting-models} shows the main effects of another model of the pareto front.
We chose a model that maximizes the following tradeoff:

$$\text{Performance} + \frac{1}{3}(\text{Simplicity} + \text{Additivity} + \text{Sparsity}) $$
...
best.tradeoff.model = best.models.print[which(tradeoff_measure == max(tradeoff_measure)), ][1,]

pareto_index = best.tradeoff.model$index
...
The selected model is a \Sexpr{best.tradeoff.model$selected.learner} using \Sexpr{best.tradeoff.model$y_4} features, an MAE of \Sexpr{best.tradeoff.model$y_1}, \Sexpr{best.tradeoff.model$y_3} variance explained due to interactions and average complexity of \Sexpr{best.tradeoff.model$y_2}.
