...
#    makeNumericParam("eta", lower = -10, upper = -2, trafo = function(x) 2^x)
  ...
  regr.ranger = makeParamSet(
    makeIntegerParam("mtry", lower = 1, upper = ncol(task.dat) - 1),
    makeIntegerParam("num.trees", lower = 1, upper = 700),
    makeIntegerParam("min.node.size", lower = 1, upper = round(nrow(task.dat)/10))
  ),
...
The goal is to predict wine quality on a scale of 0 to 10, assessed by the median of three blind ratings.

% =============================================================================
% Why like this?
% =============================================================================
\subsubsection{Motivation.}
As \citep{freitas2014comprehensible} emphasizes, it is difficult to know the desired compromise between interpretability and performance before the modeling step and suggests multi-objective optimization.
...
We used a model-based optimization framework \citep{mlrMBO} to find the best model based on four objectives: number of features used by the model (NF), main effect complexity (MEC), interaction strength (IAS) and the mean absolute error (MAE) on unseen data.
We optimized over the space of following model classes (and hyperparameters): CART (maximum tree-depth, pruning cp), support vector machine (cost C and sigma), ridge regression (regularization alpha and penalization lambda), gradient boosted trees (maximum depth, number of iterations), gradient boosted generalized additive model (number of iterations) a random forest  (mtry, number of trees, mininmal node size).
...
If two models from the same class but with different hyperparameter had exactly the same MAE, NF, IAS and MEC, we randomly dropped one from the Pareto set.
We also dropped constant models (e.g. ridge regression with strong penalization), with exception of the median model.
...
rownames(tab) = c("MAE", "NF", "IAS", "MEC", setdiff(colnames(task.dat), "quality"))

cap = sprintf("A selection of %i models from the Pareto optimal set. From left to right, the models with best MAE, best MAE when $MEC \\leq 2$, best MAE when $IAS =\\leq 0.1$, best MAE with $NF \\leq 7$. Hyperparameters are in Table~\ref{tab:pareto}.", length(indices))
...
We mapped each measure to the interval $[0,1]$ by scaling each measure with meaningful upper and lower bounds:
...
To combine the tree measures in a single dimension in Figure~\ref{fig:perf-interpret-tradeoff}, we defined interpretability ad-hoc as: $Interpretability = 3 - (NF_{scaled} + IAS_{scaled} + MEC_{scaled})$
This weights all three (scaled) measures equally.
The maximum interpretability is 3 for the model that always predicts the median wine quality.
The theoretical minimum interpretability is 0 for a model that uses all features and has the highest interaction strength and main effect complexity measure among all Pareto optimal models.
