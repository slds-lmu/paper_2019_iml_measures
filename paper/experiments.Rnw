...
<<eval = TRUE, cache = FALSE, results = "hide">>=
library(parallel)
...
@
Table \ref{tab:cor} describes the correlations between the performance and complexity measures across the \Sexpr{length(tasks)} datasets and \Sexpr{length(learners)} models.
