...
<<>>=
set.seed(123)
superconductor = read.csv("../data/superconductor.csv")
superconductor = superconductor[sample(1:nrow(superconductor), 1000), ]
sc.task = makeRegrTask(data = superconductor, target = "critical_temp")
...
airquality.task = makeRegrTask(data = airquality, target = "Ozone")
...
wine$type = NULL
wine.task = makeRegrTask(data = wine, target = "quality")
...
The dataset are:
Bike Rentals (\Sexpr{nrow(bike)} rows, \Sexpr{ncol(bike) - 1} features)
Boston Housing (\Sexpr{nrow(bh)} rows, \Sexpr{ncol(bh) - 1} features),
Superconductor (\Sexpr{nrow(superconductor)} rows, \Sexpr{ncol(superconductor) - 1} features),
Abalone (\Sexpr{nrow(abalone)} rows, \Sexpr{ncol(abalone) - 1} features),
...
dig = c(0, 0, c(0, 2, 2, 0), rep(c(2, 2, 2, 0), times = 3))
xtab = xtable(tab, align = al, caption = "Model performance and complexity for various learners on various datasets", digits = dig, label = "tab:exp", 
	      size="\\fontsize{9pt}{10pt}\\selectfont")
print(xtab, add.to.row = addtorow, include.colnames = FALSE, include.rownames = FALSE)
@

Table \ref{tab:exp} shows the performance and complexity measures.
As desired, the mean effect complexity for (regularized) linear models is always 1, and higher for more flexible methods like boosted models or random forest.
...
For example, the longley dataset seems to have purely additive effects, since even random forests, which often model strong interactions don't cover interactions.
In comparison the models that predict the wine dataset the best include a lot of interactions.

TODO: Describe results
TODO: Decrease size of font in table
TODO: Give data task names for table
TODO: Describe and rename models/learners
TODO: describe number of features with cat / num
TODO: Check settings
<<eval = FALSE>>=
ggplot(results.m) +
  geom_point(aes(x = value, y = learner)) +
  geom_segment(aes(x = 0, xend = value, y = learner, yend = learner)) + 
  facet_grid(variable ~ task, scale = "free_y")
@
