...
Steps:
$$\underbrace{\text{Black box}}_{\text{measure n.features}} \Rightarrow \text{Decomposition} \underbrace{\Rightarrow}_{\text{measure fit}} \underbrace{\text{Simple Model / Reduced decomposition}}$$

We propose measures that depend on the model  function (ignoring internal parameter) in combiniation with how we decompose it.
