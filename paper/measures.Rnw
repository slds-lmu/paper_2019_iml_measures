...
=&\underbrace{f_0 + \sum_{j=1}^p \overbrace{f_j(x_j)}^\text{AMEC: How complex?} + \overbrace{IA(x)}^{\text{IA: How much interaction?}}}_{\text{NF: How many feature used?}}
\end{eqnarray}
...
% =============================================================================
Based on the ALE decomposition, we estimate  the constant with the mean predictions $f_0 = \frac{1}{n}\sum_{i=1}^n \hat{f}(\xi)$ and the main effects with first order ALE plots $f_j(x_j) = f_{j, ALE}(x_j)$
...
\begin{eqnarray}
\tilde{f}_{j,ALE}(x_j) = \int_{z_{0,j}}^{z_j}\int P_{-j|j}(x_{-j}|z_j)\frac{\delta f(z_j, x_{-j})}{\delta z_j}(z_j, x_{-j})dx_{-j}dz_j
\end{eqnarray}
