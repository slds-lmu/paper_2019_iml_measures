...
\item Formally define: Sparsity, Additivity, Performance, Simplicity (only possible if you have mutliple models)
\item Call $R^2$ approximation of first order model "Fidelity"?
\end{itemize}
...
\item Show for a few ML methods how parameters map to the measures.
\end{itemize}
